...
import { ensureCopilotSessionScan } from './fileWatcher.js';
...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
	...
	sessionPollTimers = new Map<number, ReturnType<typeof setInterval>>();
	permissionTimers = new Map<number, ReturnType<typeof setTimeout>>();

	// Session scan state
	knownSessionFiles = new Set<string>();
		...
		this.webviewMessageDisposable = webviewView.webview.onDidReceiveMessage(async (message) => {
			...
			if (message.type === 'openCopilot' || message.type === 'openClaude') {
				...
				await openCopilotChat(
					this.nextAgentId,
					this.agents, this.knownSessionFiles,
					this.fileWatchers, this.pollingTimers, this.waitingTimers, this.permissionTimers,
					this.sessionPollTimers, this.projectScanTimer,
				...
				removeAgent(
					id, this.agents,
					this.fileWatchers, this.pollingTimers, this.waitingTimers, this.permissionTimers,
					this.sessionPollTimers, this.persistAgents,
				);
				...
				if (sessionsDir) {
					ensureCopilotSessionScan(
						sessionsDir, this.knownSessionFiles, this.projectScanTimer,
